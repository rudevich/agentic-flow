...
import { ownPackage, packageRoot } from '../../project.ts';
import { tmpProject, writeJson } from '../helpers.ts';

const escaped = (version: string) => version.replace(/\./g, '\\.');

function writeManifest(root: string, extra: Record<string, unknown>) {
 ...
 * The source, not dist/: Node runs it as it is, and the suite needs no build.
 */
function runPostinstall(env: Record<string, string>) {
  return execFileSync(process.execPath, [path.join(packageRoot, 'src', 'commands', 'postinstall.ts')], {
    encoding: 'utf8',
