...
const scaffold = async () => {
  ...
  await silenced(() => init({ cwd: root }));
  return root;
};

const run = (root: string, options: Parameters<typeof reset>[0] = {}) =>
  silenced(() => reset({ cwd: root, yes: true, ...options }));
...
describe('reset', () => {
  ...
  it('shrinks the manifest to the kept files instead of deleting it', async () => {
    ...
    const entries = defined(readManifest(root)).entries.filter((e) => e.type === 'file');
