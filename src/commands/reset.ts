...
import { cyan, dim, red } from '../platform/color.ts';
import { AGENTIC_DIR, ENV_FILE, GITIGNORE_FILE, MANIFEST_PATH } from '../constants.ts';
import { createReporter, inside, removeIfEmpty, removePath, statOrNull } from '../platform/fsx.ts';
import { readManifest, type ManifestData, type ManifestEntry } from '../model/manifest.ts';
...
export interface ResetResult {
  removed: number;
  kept: number;
  /** How many paths a guard stopped: uncommitted changes, or no confirmation. */
  blocked?: number;
}

/** Something reset takes out: a manifest entry, or with --all the whole agentic/ tree. */
type Removal = ManifestEntry | { path: string; type: 'tree' };

/** An entry reset leaves in place, and why. */
type Kept = ManifestEntry & { reason: 'modified' | 'repointed' };

/** Untracked or uncommitted content is unrecoverable once deleted. */
function dirtyPaths(root: string, paths: string[]): string[] {
  try {
...
function plan(
  ...
  { force, secrets, all }: { force: boolean; secrets: boolean; all: boolean },
): { remove: Removal[]; kept: Kept[] } {
...
function dropGitignoreLine(root: string, line: string, dryRun: boolean): boolean {
