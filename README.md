...
Скану видны четыре места: проектный `.mcp.json`, `~/.claude.json` (глобальные и
привязанные к проекту), `~/.claude/settings*.json` и `.mcp.json` включённых
плагинов. Коннектор, живущий где-то ещё, не увидится — роль останется пустой,
хотя сервер у вас есть.
...
```bash
npm update @rudevich/agentic-flow && npx agentic-flow init
```
